...
export default function App() {
  ...
  const handleMobileMenuNavigation = (screen: string) => {
  ...
  const navigateToScreen = (screen: string) => {
    ...
    navigationManager.navigateWithHistory(screen as Screen, setCurrentScreen);
  };

  // Enhanced back navigation
  const goBack = () => {
    navigationManager.goBack(setCurrentScreen);
  };
  ...
  const handleLogin = (userData: User) => {
    ...
    try {
      ...
      const userWithDefaults = enhanceUserWithDefaults(userData);
    ...
    if (!chatThread) {
      ...
      if (!chatThread.participants.includes(user.id)) {
        ...
        const joinMessage: ChatMessage = {
          ...
          senderName: "System",
          senderRole: "sender",
          message: `${user.name.split(" ")[0]} has joined the chat`,
          timestamp: new Date().toISOString(),
          type: "system",
          read: false,
        };
  ...
  const handleAcceptedBidsJobSelect = (job: DeliveryJob) => {
    ...
    handleJobSelect(job);
    navigationManager.handlePalJobNavigation(job, setCurrentScreen);
  };
  ...
  const handleFloatingActionClick = () => {
    ...
    navigationManager.handleRoleBasedNavigation(activeRole, setCurrentScreen);
  };
            ...
            onSendMessage={(message) => {
              if (selectedChatThread && user) {
                const newMessage: ChatMessage = {
                  id: Date.now().toString(),
                  senderId: user.id,
                  senderName: user.name.split(" ")[0], // Use first name only
