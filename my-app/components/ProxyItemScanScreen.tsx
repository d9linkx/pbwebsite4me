...
import { ArrowLeft, QrCode, Package, Camera, CheckCircle, Copy, Repeat } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { toast } from 'sonner';
import { FourDigitCodeInput } from './FourDigitCodeInput';

interface ProxyItemScanScreenProps {
  onBack: () => void;
  onScanComplete: () => void;
  itemTitle: string;
  deliveryId?: string;
...
export function ProxyItemScanScreen({ 
  onBack, 
  onScanComplete, 
  itemTitle,
  deliveryId = 'N/A',
  myHandoverCode = `PRAWN-${Date.now().toString().slice(-6)}`
}: ProxyItemScanScreenProps) {
  ...
  const handleCodeVerified = (code: string) => {
    ...
    // Auto-accept after code verification
    setTimeout(() => {
      onScanComplete();
    }, 1000);
  };
      ...
      >
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-semibold text-white">Proxy Item Verification</h1>
            <p className="text-sm text-gray-400">Scan and verify incoming item</p>
          </div>
        </div>
      </motion.div>
          ...
          {mode === 'scan' ? (
              ...
              {!scannedCode ? (
                ...
                >
                  ...
                  <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 rounded-2xl p-8 text-center">
                    ...
                    <p className="text-green-200 text-sm mb-4">Accepting item automatically...</p>
            ...
            >
              ...
              <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6">
                <p className="text-gray-400 text-sm mb-3 text-center">My Proxy Code:</p>
              ...
              <div className="bg-blue-500/20 border border-blue-500/30 rounded-2xl p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-blue-300 mb-2">As a Proxy:</h4>
                    <ul className="text-sm text-blue-200 space-y-1 list-disc list-inside">
                      <li>Show this code to the Pal for verification</li>
                      <li>Verify item matches description</li>
                      <li>Check for any visible damage</li>
                      <li>Store item in secure location</li>
                    </ul>
                  </div>
                </div>
              </div>
