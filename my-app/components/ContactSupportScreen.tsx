    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#2f2f2f] via-[#1a1a1a] to-[#2f2f2f] flex flex-col items-center justify-center p-6">
        ...
        >
          ...
          <p className="text-gray-400 mb-2">Reference: #{Math.random().toString(36).substring(2, 9).toUpperCase()}</p>
          <p className="text-gray-400 mb-8 max-w-md">
            Our support team will review your request and get back to you within 24 hours.
          </p>
          <motion.button
            onClick={onBack}
            className="bg-[#f44708] hover:bg-[#ff5722] text-white px-8 py-3 rounded-xl font-semibold flex items-center justify-center"
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#2f2f2f] via-[#1a1a1a] to-[#2f2f2f] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 relative z-10">
        ...
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
        >
          <h3 className="text-white font-semibold mb-4">What do you need help with?</h3>
            ...
            {categories.map((category, index) => (
              <motion.button
                ...
                className={`p-4 rounded-xl border-2 transition-all ${
                  selectedCategory === category.id
                    ? 'border-[#f44708] bg-[#f44708]/20 scale-[1.02]'
              ...
              >
                <span className={`font-medium ${
                  selectedCategory === category.id ? 'text-[#f44708]' : 'text-white'
                }`}>
                  {category.label}
                </span>
              </motion.button>
        ...
        >
          <label className="text-white font-semibold mb-3 block">Subject</label>
          <Input
            value={subject}
            onChange={(e) => setSubject(e.target.value)}
            placeholder="Brief description of your issue"
            className="bg-white/10 border-white/20 text-white placeholder-gray-500 focus:border-[#f44708]"
          />
        </motion.div>
        ...
        >
          <label className="text-white font-semibold mb-3 block">Message</label>
          <Textarea
            value={message}
            onChange={(e) => setMessage(e.target.value.slice(0, 1000))}
            placeholder="Describe your issue in detail..."
            className="bg-white/10 border-white/20 text-white placeholder-gray-500 focus:border-[#f44708] min-h-[150px]"
            rows={6}
          />
          <p className="text-xs text-gray-400 mt-2 text-right">{message.length}/1000</p>
        </motion.div>
        ...
        >
          <h3 className="text-white font-semibold mb-4">Attachments (Optional)</h3>
            ...
            <div className="space-y-2 mb-4">
              <AnimatePresence>
                {attachments.map((attachment) => (
                  <motion.div
                    key={attachment.id}
                    className="flex items-center justify-between bg-white/10 rounded-xl p-3 group"
