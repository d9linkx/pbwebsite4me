...
import { ArrowLeft, QrCode, Package, Camera, CheckCircle, Copy, User } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { toast } from 'sonner';
import { FourDigitCodeInput } from './FourDigitCodeInput';

interface ProxyToReceiverHandoverScreenProps {
  onBack: () => void;
  onHandoverComplete: () => void;
  itemTitle: string;
  deliveryId?: string;
  receiverName?: string;
  myHandoverCode?: string;
}

export function ProxyToReceiverHandoverScreen({ 
  onBack, 
  onHandoverComplete, 
  itemTitle,
  deliveryId = 'N/A',
  receiverName = 'Receiver',
  myHandoverCode = `PRAWN-${Date.now().toString().slice(-6)}`
}: ProxyToReceiverHandoverScreenProps) {
  const [mode, setMode] = useState<'scan' | 'show'>('show'); // Default to 'show' since proxy typically shows code
  ...
  const handleScanDemo = () => {
    ...
    setTimeout(() => {
      ...
      setIsScanning(false);
      toast.success('Receiver code verified successfully');
      // Auto-complete handover after scan
      setTimeout(() => {
        onHandoverComplete();
      }, 1000);
    }, 2000);
  ...
  const handleCodeVerified = (code: string) => {
    setScannedCode(code);
    toast.success('Receiver code verified successfully');
    // Auto-complete handover after code verification
    setTimeout(() => {
      onHandoverComplete();
    }, 1000);
  };
      ...
      >
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-semibold text-white">Handover to Receiver</h1>
            <p className="text-sm text-gray-400">Verify handover with {receiverName}</p>
          </div>
        </div>
      </motion.div>
          ...
          {mode === 'scan' ? (
            ...
            >
              ...
              {!scannedCode ? (
                <>
                  {/* Scanner View */}
                  <motion.div 
                    className="w-full aspect-square bg-white/10 backdrop-blur-sm border-2 border-dashed border-white/30 rounded-2xl overflow-hidden relative"
                  ...
                  >
                    <div className="absolute inset-0 flex items-center justify-center">
                      <motion.div
                        className="w-64 h-64 border-4 border-blue-500 rounded-2xl relative"
                        animate={{
                          boxShadow: [
                      ...
                      >
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          {isScanning ? (
                            ...
                            >
                              <Camera size={48} className="text-blue-500/50" />
                            </motion.div>
                          ) : (
                            <QrCode size={48} className="text-blue-500/50" />
                          )}
                        </div>
                      </motion.div>
                    </div>
                  ...
                  >
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium text-blue-300 mb-2">Scan Receiver&apos;s Code:</h4>
                        <p className="text-blue-200 text-sm leading-relaxed">
                          Position the receiver&apos;s QR code within the frame to verify the handover.
                        </p>
                      </div>
                    </div>
                  </motion.div>
                  ...
                  >
                    <FourDigitCodeInput 
                      ...
                      onCancel={() => {}}
                      placeholder="Enter receiver's code"
                    />
                  </motion.div>
                </>
              ) : (
                ...
                >
                  ...
                  <div className="bg-gradient-to-r from-green-500/20 to-blue-500/20 border border-green-500/30 rounded-2xl p-8 text-center">
                    ...
                    <p className="text-green-200 text-sm mb-4">Completing handover automatically...</p>
            ...
            >
              ...
              <motion.div 
                ...
                transition={{ delay: 0.2 }}
              >
                <p className="text-gray-400 text-sm mb-3 text-center">Handover Code for Receiver:</p>
                ...
                <p className="text-gray-500 text-xs text-center mt-3">
                  This code was also sent to {receiverName} via WhatsApp/SMS
                </p>
              </motion.div>

              {/* 4-Digit Code Input for Receiver */}
              <motion.div 
              ...
              >
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <div>
                    <h4 className="text-green-200 font-medium">Receiver Has the Code?</h4>
                    <p className="text-green-300 text-sm">They can enter it below to confirm</p>
                  </div>
                </div>
                <FourDigitCodeInput 
                  onCodeVerified={handleCodeVerified}
                  onCancel={() => {}}
                  placeholder="Receiver enters code"
                />
              </motion.div>
              ...
              >
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-blue-300 mb-2">Handover Options:</h4>
                    <ul className="text-sm text-blue-200 space-y-1 list-disc list-inside">
                      <li>Show this QR code to the receiver for scanning</li>
                      <li>OR receiver enters the 4-digit code above</li>
                      <li>Verify item condition with receiver</li>
                      <li>Auto-completes after verification</li>
                    </ul>
                  </div>
                </div>
              </motion.div>
