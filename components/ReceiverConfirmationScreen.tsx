...
import { ArrowLeft, Package, QrCode, Camera, CheckCircle, Copy, Repeat } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { toast } from 'sonner';
import { DeliveryJob } from '../types';
import { FourDigitCodeInput } from './FourDigitCodeInput';

interface ReceiverConfirmationScreenProps {
  job: DeliveryJob | null;
  onBack: () => void;
  onConfirmReceipt: () => void;
}

export function ReceiverConfirmationScreen({ job, onBack, onConfirmReceipt }: ReceiverConfirmationScreenProps) {
  ...
  const handleScanDemo = () => {
    ...
    setTimeout(() => {
      ...
      // Auto-confirm receipt after scan
      setTimeout(() => {
        onConfirmReceipt();
      }, 1000);
    }, 2000);
  ...
  const handleCodeVerified = (code: string) => {
    setScannedCode(code);
    toast.success('Pal code verified');
    // Auto-confirm receipt after code verification
    setTimeout(() => {
      onConfirmReceipt();
    }, 1000);
  };
      ...
      >
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-semibold text-white">Confirm Receipt</h1>
            <p className="text-sm text-gray-400">Verify and accept delivery</p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 relative z-10">
        <AnimatePresence mode="wait">
          {mode === 'scan' ? (
            ...
            >
              {/* Item Info */}
              <div className="bg-gradient-to-r from-yellow-500/20 to-green-500/20 border border-yellow-500/30 rounded-2xl p-6">
                <div className="flex items-center space-x-3">
                  <Package size={24} className="text-yellow-300" />
                  <div>
                    <h3 className="text-white font-semibold">{job?.title || 'N/A'}</h3>
                    <p className="text-yellow-200 text-sm">Delivered by {job?.selectedPalName || 'Pal'}</p>
                  </div>
                </div>
              </div>
              ...
              {!scannedCode ? (
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        <p className="text-green-200 text-sm leading-relaxed">
                          Position the Pal&apos;s QR code within the frame to verify the delivery handover.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                >
                  ...
                  <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 rounded-2xl p-8 text-center">
                    ...
                    <p className="text-green-200 text-sm mb-4">Confirming receipt automatically...</p>
                  ...
                  <div className="bg-yellow-500/20 border border-yellow-500/30 rounded-2xl p-4">
                    <h4 className="font-medium text-yellow-300 mb-2">✓ Payment Release:</h4>
                    <p className="text-sm text-yellow-200">
                      By verifying this code, you acknowledge that the item was delivered in good condition. 
                      Payment will be released to the Pal.
                    </p>
                  </div>
                  ...
                  <div className="flex justify-center">
                    ...
                    >
                      {[...Array(3)].map((_, i) => (
                        <motion.div
                          key={i}
                          className="w-3 h-3 rounded-full bg-gradient-to-r from-yellow-500 to-green-500"
                          animate={{
                            y: [-10, 0, -10],
                            opacity: [0.5, 1, 0.5]
                          }}
            ...
            >
              ...
              <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6">
                <p className="text-gray-400 text-sm mb-3 text-center">My Receipt Code:</p>
              ...
              <div className="bg-blue-500/20 border border-blue-500/30 rounded-2xl p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-blue-300 mb-2">Receipt Verification:</h4>
                    <ul className="text-sm text-blue-200 space-y-1 list-disc list-inside">
                      <li>Show this code to the Pal</li>
                      <li>Pal can scan to verify handover</li>
